...
## 四、20 分钟快进快出：定义、分层与识别标准

### 4.1 正式定义（推荐采用）

> **20 分钟快进快出候选事件（fast candidate） = 在同一个 `eventSlug` 内，任意 20 分钟滚动窗口中，同时出现 BUY 和 SELL，且不是轻微抖动。**
...
- 该定义用于**发现快行为**；
- 不是所有命中的事件都应直接排除；
- 更合理的做法是：先标记为 `fast candidate`，再判断它究竟是“快但可跟”，还是“高危快交易”。

### 4.2 候选快事件（fast candidate）判定口径

对每个 `eventSlug`，做滚动 20 分钟窗口，满足以下条件则记为一次候选快事件：
...
所以更推荐使用**话题级 20 分钟买卖同窗**定义，作为第一层筛选。

### 4.4 候选快事件的二次判定：区分“快但可跟”与“高危快交易”

候选快事件命中后，不建议直接一票否决，而应进一步评估其**可复制性**。

建议至少计算以下指标：

- `first_sell_lag_sec`
  - 第一个 SELL 时间 - 第一个 BUY 时间
- `window_span_sec`
  - 最后一笔 SELL 时间 - 第一笔 BUY 时间
- `buy_trade_count`
- `sell_trade_count`
- `buy_max_trade_share`
  - 最大单笔 BUY / 窗口内总 BUY
- `sell_max_trade_share`
  - 最大单笔 SELL / 窗口内总 SELL
- `fast_turnover_ratio`
  - 窗口内 `(BUY+SELL)` / 该 `eventSlug` 全周期 `(BUY+SELL)`
- `repeated_fast_windows_same_event`
  - 同一 `eventSlug` 命中的候选快窗口数量

### 4.5 高危快交易（non-copyable fast）

满足候选快事件后，若再满足以下条件中的 **至少 2 条**，建议定义为**高危快交易**：
...
### 4.6 快但可跟（actionable fast）

满足候选快事件，但**不满足高危快交易定义**，则可归类为**快但可跟**。

典型特征：

- `first_sell_lag_sec >= 180`
- `window_span_sec >= 480`（至少 8 分钟）
- `buy_trade_count >= 2`
- `sell_trade_count >= 2`
...
- 存在一定跟单窗口
- 不是由单笔大单瞬间完成
- 更像“快调仓”，而不是“不可复制的快冲快跑”

### 4.7 使用原则

因此，本标准对 20 分钟快进快出的推荐用法是：

1. **先用 `fast candidate` 发现快行为**
2. **再用二次判定识别 `high-risk fast`**
3. 真正用于账户准入和题材黑名单的，应优先看：
   - `high-risk fast` 占比
   - `fast candidate` 中有多少能够归为 `actionable fast`
...
4. `fast_event_20m_event_ratio`
   - = 命中候选快事件的 `eventSlug` 数量 / 总 `eventSlug` 数量
...
- `fast_event_20m_*` 用来衡量账户里“快行为有多少”
- `noncopyable_fast_*` 用来衡量账户里“真正不适合跟的快交易有多少”
...
- `fast_event_20m_*` 高，并不自动等于不可跟；
- 只有当快事件里大部分都属于 `high-risk fast` 时，才应直接排除。
...
- `fast_event_20m_buy_usdc_ratio` 在 **10% – 40%** 之间，但 `noncopyable_fast_*` 未明显超线
- `weighted_median_holding_time` 为**小时级**，且短线利润占比不小
...
- 必须做题材黑名单
- 必须跳过结构化事件
- 对 20 分钟同话题买卖事件，应进一步区分：
  - `actionable fast`：可保留观察
  - `high-risk fast`：建议跳过
...
- 快事件存在，但大部分属于 `actionable fast`
- 短时间双边切换很少
- 持有时间以**数小时到数天**为主
- 主体行为是正常买入后等待再定价，再卖出获利/止损

这类账户依然不等于可以无脑跟，但至少**主体行为更接近可复制**。
...
若 `noncopyable_fast_*` 明显超阈值，或结构化指标显著恶化，可直接排除。
...
- **结构化套利 / 结构化交易占比高** → 不适合跟
- **高危快交易占比高** → 不适合跟
- **20 分钟同话题快进快出高，但大多属于可跟的快调仓** → 需要细分，不应直接一票否决
