...
## 1. 执行结论
校准后决策分 71.49（锚点口径），结论：只适合筛着跟。优势：可利用事件覆盖广、加权多子市场风险较低、不可复制快交易比例较低。硬黑名单主题（禁止跟）：jazz、blazers、trail、hawks、grizzlies。白名单主题（优先筛选）：magic、grizzlies、timberwolves、hawks、kings。收益曲线标签：长期与近期均偏弱。建议仅在严格事件筛选和黑名单约束下筛选着跟。

## 2. 决策快照
- decision: `只适合筛着跟`
- final_score（决策分）: `71.490000`
- raw_score: `64.980000`
...
- 硬黑名单主题（应避免）：jazz, blazers, trail, hawks, grizzlies.
