...
## 1. 执行结论
校准后决策分 30.92（锚点口径），结论：不值得跟。主要板块暴露：us_politics、geopolitics。优势：加权多子市场风险较低。风险点：递进型并存梯度风险偏高、存在不可复制的 token 快交易暴露。硬黑名单主题（禁止跟）：say、trump、during、march、week。软黑名单主题（谨慎跟）：reform、prime、starmer、minister、questions。风险门槛已触发，宽跟模式自动关闭。重风险门槛已触发，低分情形会被强制判定为不值得跟。收益曲线标签：长期与近期均偏弱。不建议作为主跟单源，仅可少量人工挑选。

## 2. 决策快照
- decision: `不值得跟`
- final_score（决策分）: `30.920000`
- raw_score: `2.570000`
...
- 硬黑名单主题（应避免）：say, trump, during, march, week, fury.
- 软黑名单主题（需更严格触发）：reform, prime, starmer, minister, questions.
