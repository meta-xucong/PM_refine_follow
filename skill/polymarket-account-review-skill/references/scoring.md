...
- `pnl_curve_stability_score`: `-15..+15` (converted into `0..15` contribution by offset in implementation)
- `risk_penalty_adjustment`: `-20..0`
...
- `exclusive_concurrent_leg_ratio > 0.35`
- `nested_concurrent_leg_ratio > 0.40` and `event_rebalance_20m_event_ratio >= 0.15`
- `weighted_multi_market_risk_ratio > 0.50` and (`exclusive_concurrent_leg_ratio > 0.20` or `nested_concurrent_leg_ratio > 0.25`)
- `noncopyable_token_fast_buy_ratio > 0.20`
- `noncopyable_token_fast_sell_ratio > 0.25`
- `noncopyable_token_fast_token_ratio > 0.20`
- `dual_side_buy_usdc_ratio > 0.40`
- `dual_side_buy_usdc_ratio_1h > 0.20`
...
- if `deployable_event_equivalent < 3` OR `deployable_event_density < 0.10`: cap `55`
- else if `< 5` OR `< 0.17`: cap `62`
- else if `< 8` OR `< 0.26`: cap `70`

## 5) Decision Mapping

- Anchor-adjusted score (`anchored_score`) is used for threshold mapping:
